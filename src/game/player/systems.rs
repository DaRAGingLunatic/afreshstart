...
use crate::game::star::systems::HALF_STAR_SIZE;
...
pub fn spawn_player(
    ...
    // get single returns the Result type. We unwrap the result type to get the reference to the window.
...
pub fn player_movement(
    ...
    if let Ok(mut transform) = player_query.get_single_mut() {
        ...
        // we set the direction to zero as we are not sure which direction we wish to move in yet.
        ...
        // println!("Direction: {:?}", direction.length_squared());
    }
}
...
pub fn confine_player_movement(
    ...
    if let Ok(mut player_transform) = player_query.get_single_mut() {
        ...
        let x_max = window.width() - (HALF_PLAYER_SIZE + 4.0);
        let y_min = HALF_PLAYER_SIZE;
        let y_max = window.height() - (HALF_PLAYER_SIZE + 33.0);
